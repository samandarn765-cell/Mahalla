  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white dark:bg-[#0A0F1D] border-b border-gray-200 dark:border-[#1A2235] transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        {/* Brand Logo & Name (Desktop) and Mobile Top Bar */}
        <div className="flex items-center gap-3 select-none">
          {/* Mobile Profile Avatar */}
          <Link to="/profile" className="lg:hidden w-8 h-8 rounded-full overflow-hidden border border-gray-600/50 shadow-inner shrink-0">
            {userData?.avatar ? (
              <img src={userData.avatar} alt="Profile" className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full bg-slate-800 flex items-center justify-center">
                <img src={logoImg} alt="Default Profile" className="w-full h-full object-cover p-1 opacity-70" />
              </div>
            )}
          </Link>
          ...
          {/* Mobile Title */}
          <Link to="/" className="lg:hidden ml-1">
        ...
        <div className="flex items-center gap-3 relative">
          ...
          <button
            ...
            className="p-2 text-gray-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800 rounded-xl transition-colors"
          >
            <Languages className="w-5 h-5 hidden lg:block" />
            <Languages className="w-5 h-5 lg:hidden text-emerald-600 dark:text-indigo-200" />
          </button>
          ...
          <NavLink
            to="/profile"
            className={({ isActive }) =>
              `hidden lg:flex p-2 px-3 rounded-xl transition-all items-center gap-2 ${
                isActive
      ...
      {/* Mobile Navigation is handled by MobileBottomNav */}
    </header>
  );
};
